 ...
 *
 * 세션/쿠키 주의
 * - CORS 환경에서는 서버의 `cors({ origin, credentials:true })`와 프론트의 `fetch(..., { credentials:'include' })`
 *   또는 Axios의 `withCredentials:true` 설정이 반드시 필요합니다.
 * - 프록시/도메인 설정에 따라 `cookie`의 `domain`, `sameSite`, `secure`를 운영 환경에서 조정하세요.
 *
 * 현재 저장소
 * - DB(Prisma) 연동 버전으로 가정합니다. (팀 상황에 따라 인메모리 저장을 사용할 수도 있음)
 * - DB 스키마/비밀번호 해시는 service 레이어가 책임지며, 컨트롤러는 세션/응답만 담당합니다.
 */
...
/**
 * 비밀번호/해시 등 민감정보를 제거하고 프론트로 내려보낼 사용자 객체로 변환
 * - name 이 없으면 email 앞부분을 name 으로 사용
 * - role 기본값은 'user'
 * 
 * 사용 이유:
 * 1) DB 레코드에는 passwordHash 등 민감정보가 포함될 수 있으므로,
 * 2) 컨트롤러는 항상 "안전한(민감정보 제거된) 구조"로만 프론트에 응답하기 위해 변환을 거칩니다.
 */
 ...
 *  - 프론트에서도 유효성 검사를 하더라도, 백엔드에서 1회 더 방어적으로 검사합니다.
 */
...
export const me = (req: Request, res: Response) => {
  // (선택) 인증 미들웨어가 req.currentUser 를 주입했다면 우선 사용, 없으면 세션 사용자 사용
...
export const register = async (req: Request, res: Response) => {
  ...
  // express-session 으로 주입되는 객체. 없으면 서버 설정 문제(CORS/미들웨어 누락 등)
  ...
  try {
    ...
    sess.regenerate?.((regenErr) => {
      ...
      return res.status(201).json({ ok: true, user: (req as any).session.user });
    });

    // regenerate 가 없는 환경(메모리스토어 등)은 바로 세팅
    if (!sess.regenerate) {
      (req as any).session.user = toPublicUser(created)!;
      return res.status(201).json({ ok: true, user: (req as any).session.user });
    }
...
export const login = async (req: Request, res: Response) => {
  ...
  try {
    // 서비스 레이어에서 이메일/비밀번호 검증 → 실패 시 null
    ...
    sess.regenerate?.((regenErr) => {
      ...
      return res.status(200).json({ ok: true, user: (req as any).session.user });
    });
 ...
 *
 * 운영 팁:
 * - 리버스 프록시/도메인 구성에 따라 `res.clearCookie` 의 옵션(domain/sameSite/secure) 조정이 필요할 수 있습니다.
...
export const logout = (req: Request, res: Response) => {
  ...
  // 세션이 없다면 쿠키만 정리하고 성공 처리(서버 설정에 따라 세션 미들웨어가 없을 수도 있으므로 방어)
  if (!sess) {
