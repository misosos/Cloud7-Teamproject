 ...
 * ================================================================================
 * 역할
 *  - 공통 미들웨어(CORS, 로깅, JSON/FORM 파싱, 쿠키/세션, 보안/압축)를 한 곳에 모아
 *    Express 앱 인스턴스를 생성합니다.
 *  - 실제 서버 시작(포트 바인딩)은 `src/server.ts`가 담당하고,
 *    이 파일은 오직 "앱 구성(app configuration)"만 담당합니다.
 *
 * 요청 처리 흐름(요약)
 ...
 *     - `express.json()` / `express.urlencoded()`으로 요청 본문 파싱
 *     - `cookie-parser`로 쿠키 파싱
