  ...
  router.get(
    ...
    // ============================================================
// [GET] /api/taste-records/dashboard
// ------------------------------------------------------------
// Stay 기반으로 취향 비율(카테고리 7개)을 계산하고,
// TasteRecord 테이블에 category='DASHBOARD' 레코드로 스냅샷 저장한 뒤,
// weights + totalStays + dashboardRecord 를 응답합니다.
// ------------------------------------------------------------
// 프론트 예시:
//   fetch('/api/taste-records/dashboard', { credentials: 'include' })
//     .then(r => r.json())
//     .then(console.log);
// ============================================================
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      const result = await buildTasteDashboardFromStays(userId);

      res.json({
        ok: true,
        totalStays: result.totalStays,
        weights: result.weights,
        dashboardRecord: result.dashboardRecord,
      });
    } catch (err) {
      next(err);
    }
